...
- Deployed live to Streamlit Community Cloud from `main` (Cloud installs from `uv.lock`): https://chunk-lab.streamlit.app . Both tabs verified working live, including a full eval run (embed + Chroma) on the free tier. Note: Cloud hot-reload does not re-import changed submodules, so a push that adds names to `chunklab/*` may need a one-time app reboot; Stage 2 symbols are imported lazily so a stale submodule can't crash Stage 1.
